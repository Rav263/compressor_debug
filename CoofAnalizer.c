...
int compress_ari(uint16_t*, char*, int, int, int, double*);
        ...
        for (int freq_max = freq_max_min; freq_max < end; freq_max += coof / 10) {
            ...
            for (int freq_mult = 1; freq_mult < 72; freq_mult += mult_cof) {
                for (int null_const = 2; null_const < 73; null_const += mult_cof) {
                    double mt_wt;
                    int size = compress_ari(some, out_name, freq_max, freq_mult, null_const, &mt_wt);
                    ...
                    if ((size < min_file_size)|| min_file_size == -1) {
                        ...
                        fprintf(stderr, "max:%d mult:%d null:%d size:%d\n", freq_max, freq_mult, null_const, size);
                    }
                }
            }
        }

        printf("max:%d mult:%d null:%d size:%d: approx:%lf\n", freq_max_min, freq_mult_min, null_const_min, min_file_size, max_wait);
        break;
        } else {
        }
    }
