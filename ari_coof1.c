...
void init_table(Table *table, int index) {
    ...
    table->char_freq[0] = 0;
    table->max_freq = MAX_FREQ;//max_freqs[index];
    table->null_const = NULL_CONST;//null_consts[index];
    table->freq_mult = FREQ_MULT;//freqs_mult[index];
    table->target_up  = UPDATE_CONST;
    table->up_const = CHAR_COUNT >> UP_FIR;
}
...
double compress_text(uint16_t *ifp, FILE *ofp, int *status) {
    ...
    while ((now_char = ifp[char_count]) != UINT16_MAX) {
        char_count += 1;
